...
function filterDescription(item, colorIndex, config)
{
    ...
    {
        if(item.rawPlainText.includes("Set up an E-Giving"))
        {
            item.plaintext = item.rawPlainText.substring(0, item.rawPlainText.indexOf("Set up an E-Giving"));   
        }
        else if(item.rawPlainText.includes("Missional Communities Click here for More info"))
        {
            item.plaintext = item.rawPlainText.substring(0, item.rawPlainText.indexOf("Missional Communities Click here for More info"));   
        }        
        else
        {
            item.plaintext = "";
            console.log("Description not filtered correctly, check newsletter content for format changes.")
        }    
    }
